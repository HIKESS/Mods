...
local function onhit(inst)
    if not inst:HasTag("burnt") then
        ...
        if inst.WashTask then
          inst.WashTask:Cancel()
          inst.WashTask = nil
        end
        inst.SoundEmitter:KillSound("washing")
...
local function StartWork(inst)
  ...
  if item then
    ...
    inst.WashTask = inst:DoTaskInTime(17, function()
      if inst.components.container == nil then
        return
      end
      local item2 = inst.components.container:GetItemInSlot(1)
      if item2 and item2.components.inventoryitemmoisture then
        item2.components.inventoryitemmoisture:SetMoisture(0)
      end
      inst.components.container:DropEverything()
