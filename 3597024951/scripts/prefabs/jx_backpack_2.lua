...
local function OnSeason(inst, season)
  if inst.components.insulator then
    ...
    if season == SEASONS.SUMMER or (SEASONS.DRY ~= nil and season == SEASONS.DRY) then
      ...
      if val ~= TUNING.JX_TUNING.jx_backpack_2_insulator then
        inst.components.insulator:SetInsulation(TUNING.JX_TUNING.jx_backpack_2_insulator)
      end
    end
  end
end
...
local function fn()
    ...
    --inst.components.container:EnableInfiniteStackSize(TUNING.JX_TUNING.jx_backpack_2_stacksize)
