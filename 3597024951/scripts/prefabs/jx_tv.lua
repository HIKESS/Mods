...
local function UpdateState(inst, push_anim)
    if ShouldTurnOn(inst) then
        if TUNING.JX_TUNING.jx_tv_lightenable ~= false then
...
local function fn()
    ...
    inst.Light:SetRadius(TUNING.JX_TUNING.jx_tv_lightradius) -- 5
    inst.Light:Enable(false)
...
return Prefab("jx_tv", fn, assets, prefabs),
