...
local function Change_Task(inst, turn_on)
  ...
  if turn_on then
    inst.Task = inst:DoPeriodicTask(2, function()
        ...
        local ents = TheSim:FindEntities(x, y, z, 7, nil, {"snuffed", "INLIMBO", "FX", "playerghost",}, {"player", "smolder", "fire",})
          ...
          if v:HasTag("player") then
            if v.components.health and not v.components.health:IsDead() and v.components.temperature then
              ...
              if tem > 10 then
                ...
                delta_tem = delta_tem * TUNING.JX_TUNING.jx_fan_cooling_efficiency
