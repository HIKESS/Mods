...
local function StartWork(inst)
  ...
  local worktime = TUNING.JX_TUNING.jx_icemaker_worktime
  if inst.components.timer then
    if inst.components.timer:TimerExists("cd") then
      inst.components.timer:StopTimer("cd")
    end
    inst.components.timer:StartTimer("cd", worktime)
  end
end
...
local function ontimerdone(inst, data)
  if data and data.name == "cd" then
    ...
    inst.AnimState:PushAnimation("idle"..inst.machinestate, true)
    inst:DoTaskInTime(64 * FRAMES,function()
      inst.SoundEmitter:PlaySound("dontstarve_DLC001/common/machine_fuel")
    end)
    inst:DoTaskInTime(79 * FRAMES,function()
      if inst.components.container then
