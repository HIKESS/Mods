...
local function AddRandomAtkTag(inst)
  ...
  if math.random() <= TUNING.JX_TUNING.jx_pan_aoe then
...
local function fn()
  ...
  if TUNING.JX_TUNING.jx_pan_aoe >= .8 then
    inst:AddTag(AtkTags[math.random(1, #AtkTags)].name)
  end
  ...
  ------
  if TUNING.JX_TUNING.jx_pan_eternity ~= true then
