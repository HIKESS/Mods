--这个组件与原版 autoterraformer 组件的代码、作用基本是一致的
--只在 HandleRemovingItem 函数部分有少量差异
--原版在处理物品栏中超栈堆叠的地皮物品时，会移除整组物品给予刮地皮头盔
--导致除了成功进入刮地皮头盔的20个地皮以外的剩余物品会掉落在原地
--这个组件只拿取应有的量(20个)

local JX_AutoTerraformer = Class(function(self, inst)
  self.inst = inst
...
function JX_AutoTerraformer:FinishTerraforming(x, y, z)
      ...
      if owner and owner.components.inventory and self.container:IsOpenedBy(owner) then
        ...
        if replacement then
          ...
          if self.container:CanTakeItemInSlot(replacement, targetslot) then
            if replacement.components.stackable and replacement.components.stackable:IsOverStacked() then
              replacement = replacement.components.stackable:Get(replacement.components.stackable.originalmaxsize or TUNING.STACK_SIZE_MEDITEM)
            else
...
function JX_AutoTerraformer:DoTerraform(px, py, pz, x, y)
...
function JX_AutoTerraformer:StartTerraforming()
  self.last_x, self.last_y, self.repeat_delay = nil, nil, nil
  self.inst:StartUpdatingComponent(self)
end

function JX_AutoTerraformer:StopTerraforming()
  self.inst:StopUpdatingComponent(self)
end

function JX_AutoTerraformer:OnUpdate(dt)
  ...
  if (self.last_x == nil and self.last_y == nil) or
    ...
    (self.last_x == x and self.last_y == y and self.repeat_delay == 0)
  then
