...
local events =
{
    ...
    EventHandler("gotosleep", function(inst)
		    inst.components.health:AddRegenSource(inst, (inst.components.health:GetMaxWithPenalty() * TUNING.SDF_ASGARD_GOLEM_HEALTH_SLEEP_REGEN_AMOUNT_PERCENT), TUNING.SDF_ASGARD_GOLEM_HEALTH_REGEN_PERIOD, "sdf_asgard_golem_sleep_regen")
		    inst.sg:GoToState("sleep")
		end),
    EventHandler("onwakeup", function(inst)
		    inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_SLEEP_END, 4)
		    inst.components.health:RemoveRegenSource(inst, "sdf_asgard_golem_sleep_regen")
		    inst.sg:GoToState("wake")
		end),
    EventHandler("entershield", function(inst)
		    if inst:HasTag("sdf_stone_golem_shielded_projectile") then
			inst.sg:GoToState("projectile_shield_start")
		    else
			if inst.components.sdf_asgard_golem_optimize_data:GetODTypeCInstalled() == true then
			    inst.components.health:AddRegenSource(inst, (TUNING.SDF_ASGARD_GOLEM_HEALTH * TUNING.SDF_ASGARD_GOLEM_HEALTH_SHIELD_REGEN_PERCENT), TUNING.SDF_ASGARD_GOLEM_HEALTH_REGEN_PERIOD, "sdf_asgard_golem_shield_regen")
			end
			inst.sg:GoToState("shield_start")
		    end
		end),
    EventHandler("exitshield", function(inst)
		    inst.components.health:RemoveRegenSource(inst, "sdf_asgard_golem_shield_regen")
...
local states =
{
    ...
    State{
	    ...
	    --Activate Taunt talk
	    inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_TAUNT, 4)
            inst.Physics:Stop()
            inst.AnimState:PlayAnimation("taunt")
            inst.components.timer:StartTimer("taunt_cd", math.random(TUNING.SDF_ASGARD_GOLEM_TAUNT_PERIOD_MIN, TUNING.SDF_ASGARD_GOLEM_TAUNT_PERIOD_MAX))
    ...
    State{
        name = "optimize_data_type_b_start",
        tags = {"busy"},

        onenter = function(inst)
            inst.Physics:Stop()
            inst.AnimState:PlayAnimation("rocklick_pre")
            inst.AnimState:PushAnimation("rocklick_loop", true)
            inst.sg:SetTimeout(TUNING.SDF_ASGARD_GOLEM_OPTIMIZE_DATA_TYPE_B_BARRIER_DOME_DURATION)
        end,
        ...
        ontimeout = function(inst)
            inst.sg:GoToState("optimize_data_type_b_end")
        end,
    },

    State{
        name = "optimize_data_type_b_end",
        tags = {"busy"},

        onenter = function(inst)
	    inst:HighPoweredBarrierEndFn()
    ...
    State{
	    ...
	    --Activate Optimize Data talk
	    if inst.components.sdf_asgard_golem_optimize_data:GetODTypeAInstalled() == true and inst.components.sdf_asgard_golem_optimize_data:GetODTypeCInstalled() == true then
		inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_TYPE_A_AND_TYPE_C, 4)
	    elseif inst.components.sdf_asgard_golem_optimize_data:GetODTypeAInstalled() == true then
		inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_TYPE_A, 4)
	    elseif inst.components.sdf_asgard_golem_optimize_data:GetODTypeCInstalled() == true then
		inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_TYPE_C, 4)
	    end

	    --summon lava golems -Asgard Only-
	    if inst.components.sdf_asgard_golem_optimize_data:GetODTypeAInstalled() == true and not inst:HasTag("sdf_stone_golem_shielded_projectile") then
		inst:SummonLavaGolemFn()
	    end

	    if not inst:HasTag("stealth") then
		inst:AddTag("stealth")
	    end
    ...
    State{
        ...
        onenter = function(inst)
            inst.components.health:SetAbsorptionAmount(TUNING.SDF_ASGARD_GOLEM_SHIELD_ABSORB)
    ...
    State{
	    ...
	    --Activate Attack talk
	    if inst.mode == 0 then
		inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_PUNCH_SINGLE[math.random(#STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_PUNCH_SINGLE)], 4)
	    else
		inst.components.talker:Say(STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_PUNCH_AOE[math.random(#STRINGS.ANNOUNCE_SDF_ASGARD_GOLEM_ACTIVATE_PUNCH_AOE)], 4)
	    end
...
return StateGraph("SGsdf_asgard_golem", states, events, "idle", actionhandlers)
