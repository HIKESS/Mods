...
local events=
{
    ...
    CommonHandlers.OnAttacked(nil, TUNING.SDF_ASYLUM_GROUNDS_KEEPER_MAX_STUN_LOCKS),
    ...
    EventHandler("onarrivedatthrone", function(inst)
        ...
        if not inst.sg:HasStateTag("transforming") then
            if not inst.sg:HasStateTag("sitting") and player_close == nil then
...
return StateGraph("SGsdf_asylum_grounds_keeper", states, events, "idle", actionhandlers)