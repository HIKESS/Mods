...
local function teleportahead(inst,pos)
    local nub = SpawnPrefab("sdf_pumpkin_king_vine")
    ...
    local newpos = pos
    if dist > TUNING.SDF_PUMPKIN_KING_VINE_END_MOVEDIST * TUNING.SDF_PUMPKIN_KING_VINE_END_MOVEDIST then
...
local WEAKVINE_CAN = {"weakvine","sdf_pumpkin_king"}

local MUST_TAGS =  {"_combat"}
local CANT_TAGS = { "FX", "NOCLICK", "DECOR", "INLIMBO", "invisible", "wall", "notarget", "noattack", "sdf_pumpkin_king", "sdf_pumpkin_king_vine_end","sdf_pumpkin_king_vine_segment", "sdf_pumpking_friend" }
...
local states=
{
    ...
    State{
        ...
        onenter = function(inst)
            ...
            inst.components.timer:StartTimer("attack_cooldown",TUNING.SDF_PUMPKIN_KING_VINE_END_ATTACK_PERIOD)
        end,
        ...
        timeline=
        {
            ...
            TimeEvent(17*FRAMES, function(inst)
                local x,y,z = inst.Transform:GetWorldPosition()
                local targets = TheSim:FindEntities(x, y, z, TUNING.SDF_PUMPKIN_KING_VINE_END_ATTACK_RANGE, MUST_TAGS, CANT_TAGS )
    ...
    State{
        ...
        {
            EventHandler("animover", function(inst)
                if inst:HasTag("weakvine") or not inst:HasTag("sdf_pumpkin_king_vine_end") then
                    inst.sg:GoToState("nub_idle")
                else
                    inst.sg:GoToState("idle")
                end
                if inst:HasTag("sdf_pumpkin_king_vine_end") then
                    inst:ChooseAction()
                end
            end),
        },
...
return StateGraph("SGsdf_pumpkin_king_vine", states, events, "nub_idle")
