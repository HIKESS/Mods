...
--Professors Lab, Pumpkin Gorge Well
...
AddComponentPostInit("playervision",function(LS4h)
	...
	if LS4h.currentcctable == nil then
	    --Pumpkin Gorge Well
	    if LS4h.inst._insdfpumpkingorgewellcamera ~= nil and LS4h.inst._insdfpumpkingorgewellcamera:value() ~= nil then
...
AddComponentPostInit("moisture",function(ZA9)
	...
	--Pumpkin Gorge Well
	if ZA9.inst._insdfpumpkingorgewellcamera ~= nil and ZA9.inst._insdfpumpkingorgewellcamera:value() ~= nil then
	    return 0
	end
	return hWgmxm(ZA9)
    end
...
AddComponentPostInit("birdspawner",function(nbqmx)
	...
	local pumpkinGorgeWell = TheSim:FindEntities(Q2waXkyp, EG72, mlTMZ, TUNING.BIRD_CANARY_LURE_DISTANCE, {"sdf_pumpkin_gorge_well_door"})
