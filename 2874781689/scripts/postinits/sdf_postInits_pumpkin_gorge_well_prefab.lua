local HGli = require "util/sdf_pumpkin_gorge_well_functions"
...
AddPrefabPostInit("world",function(Caz4NM4Z)
    ...
    if XVxxx then
	XVxxx.CreateSDFPumpkinGorgeWell = function(T, WZs, ITdz, AjfoUo)
	    R1FIoQI[AjfoUo] = {WZs, ITdz}
	end
	XVxxx.RemoveSDFPumpkinGorgeWell = function(Er9zidsB, X)
    ...
    Caz4NM4Z:AddComponent("sdf_pumpkin_gorge_well_limiter")
    Caz4NM4Z.components.sdf_pumpkin_gorge_well_limiter:SetName(modname)
    Caz4NM4Z:ListenForEvent("ms_playerdespawnanddelete",function(Caz4NM4Z, aBbGg)
	if aBbGg and aBbGg._insdfpumpkingorgewellcamera then
	    aBbGg.spawnanddelete_sdfpumpkingorgewell = true
	    aBbGg._insdfpumpkingorgewellcamera:set(nil)
	end
    end)
end)
