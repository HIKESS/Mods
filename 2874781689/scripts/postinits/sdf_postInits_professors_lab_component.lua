local HGli = require "util/sdf_professors_lab_functions"
...
AddComponentPostInit("playervision",function(LS4h)
	...
	if LS4h.currentcctable == nil then
	    if LS4h.inst._insdfprofessorslabcamera ~= nil and LS4h.inst._insdfprofessorslabcamera:value() ~= nil then
...
AddComponentPostInit("moisture",function(ZA9)
	...
	if ZA9.inst._insdfprofessorslabcamera ~= nil and ZA9.inst._insdfprofessorslabcamera:value() ~= nil then
	    return 0
	end
	return hWgmxm(ZA9)
    end
...
AddComponentPostInit("birdspawner",function(nbqmx)
	...
	local professorsLab = TheSim:FindEntities(Q2waXkyp, EG72, mlTMZ, TUNING.BIRD_CANARY_LURE_DISTANCE, {"sdf_professors_lab_door"})
	if #professorsLab ~= 0 and math.random() < 0.5 then
