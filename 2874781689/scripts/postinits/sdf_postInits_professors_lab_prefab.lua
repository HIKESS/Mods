...
AddPrefabPostInit("world",function(Caz4NM4Z)
    ...
    if XVxxx then
	XVxxx.CreateSDFProfessorsLab = function(T, WZs, ITdz, AjfoUo)
	    R1FIoQI[AjfoUo] = {WZs, ITdz}
	end
	XVxxx.RemoveSDFProfessorsLab = function(Er9zidsB, X)
    ...
    Caz4NM4Z:AddComponent("sdf_professors_lab_limiter")
    Caz4NM4Z.components.sdf_professors_lab_limiter:SetName(modname)
    Caz4NM4Z:ListenForEvent("ms_playerdespawnanddelete",function(Caz4NM4Z, aBbGg)
	if aBbGg and aBbGg._insdfprofessorslabcamera then
	    aBbGg.spawnanddelete_sdfprofessorslab = true
	    aBbGg._insdfprofessorslabcamera:set(nil)
	end
    end)
end)
