...
local SDFShadowDemonetteBrain = Class(Brain, function(self, inst)
...
local function IsMyTurnToAttack(inst)
	...
	return false
    elseif inst.sg.mem.lastattack ~= nil and inst.prefab == "sdf_shadow_demonette_penumbra" and (IsTheirTurnToAttack(inst, "umbra")) then
	return false
    elseif inst.sg.mem.lastattack ~= nil and inst.prefab == "sdf_shadow_demonette_umbra" and (IsTheirTurnToAttack(inst, "penumbra")) then
	return false
    end
    return true
end

function SDFShadowDemonetteBrain:OnStart()
	...
	WhileNode(function() return not IsMyTurnToAttack(self.inst) end, "WaitingTurn",
		...
		FailIfSuccessDecorator(
		    Leash(self.inst, GetTargetPos, TUNING.SDF_SHADOW_DEMONETTE_ATTACK_RANGE + 2, TUNING.SDF_SHADOW_DEMONETTE_ATTACK_RANGE - 2)),
...
return SDFShadowDemonetteBrain