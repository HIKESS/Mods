...
local function SetHarvest(inst)
    ...
    local pumpkingBomb = SpawnPrefab("sdf_pumpking_bomb")
    if pumpkingBomb ~= nil then
	pumpkingBomb.Transform:SetPosition(inst.Transform:GetWorldPosition())
	pumpkingBomb.summoned = true
	pumpkingBomb.persists = false
    end
...
for build, data in pairs(builds) do
    growth_stages[build] =
    {
        {
            name = "seed",
            time = function(inst) return TUNING.SDF_PUMPKING_BOMB_PLANT_GROWTH_TIME end,
            fn = SetSeed,
            growfn = GrowSeed,
        },
        {
        ...
        {
            name = "harvest",
            time = function(inst) return 2 end,
            fn = SetHarvest,
            growfn = GrowHarvest,
        },
    }
...
local function GetGrowthStages(inst)
    return growth_stages[inst.build] or growth_stages["sdf_pumpking_bomb_plant"]
end

local function GetBuild(inst)
    return builds[inst.build] or builds["sdf_pumpking_bomb_plant"]
end
...
local function fn()
    ...
    inst.build = "sdf_pumpking_bomb_plant"
...
return Prefab("sdf_pumpking_bomb_plant", fn, assets, prefabs)
