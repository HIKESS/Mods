...
local function OnPreLoad(inst, data)
    WorldSettings_ChildSpawner_PreLoad(inst, data, TUNING.SDF_PUMPKIN_GOURD_PLANT_RELEASE_TIME, TUNING.SDF_PUMPKIN_GOURD_PLANT_REGEN_TIME)
end
...
for build, data in pairs(builds) do
    growth_stages[build] =
    {
        {
            name = "seed",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.01, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.01) end,
            fn = SetSeed,
            growfn = GrowSeed,
        },
        {
            name = "sprout",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.1, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.1) end,
            fn = SetSprout,
            growfn = GrowSprout,
        },
        {
            name = "small",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.2, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.2) end,
            fn = SetSmall,
            growfn = GrowSmall,
        },
        {
            name = "med",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.3, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.3) end,
            fn = SetMed,
            growfn = GrowMed,
        },
        {
            name = "full",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.4, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.4) end,
            fn = SetFull,
            growfn = GrowFull,
        },
        {
            name = "oversized",
            time = function(inst) return GetRandomMinMax((TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MIN*TUNING.TOTAL_DAY_TIME) * 0.5, (TUNING.SDF_PUMPKIN_GOURD_PLANT_GROWTH_TIME_MAX*TUNING.TOTAL_DAY_TIME) * 0.5) end,
            fn = SetOversized,
...
local function GetGrowthStages(inst)
    return growth_stages[inst.build] or growth_stages["sdf_pumpkin_gourd_plant"]
end

local function GetBuild(inst)
    return builds[inst.build] or builds["sdf_pumpkin_gourd_plant"]
end
...
local function fn()
    ...
    inst.build = "sdf_pumpkin_gourd_plant"
    ...
    inst:AddTag("sdf_pumpkin_gourd_plant")
    ...
    inst.components.lootdropper:SetLoot({"sdf_pumpkin_gourd_seeds"})

    inst:AddComponent("childspawner")
    inst.components.childspawner.childname = "sdf_pumpkin_gourd"
    ...
    inst.components.childspawner:SetRegenPeriod(TUNING.SDF_PUMPKIN_GOURD_PLANT_REGEN_TIME)
    inst.components.childspawner:SetSpawnPeriod(TUNING.SDF_PUMPKIN_GOURD_PLANT_RELEASE_TIME)
    inst.components.childspawner:SetMaxChildren(TUNING.SDF_PUMPKIN_GOURD_PLANT_MAX_SPAWN)

    WorldSettings_ChildSpawner_SpawnPeriod(inst, TUNING.SDF_PUMPKIN_GOURD_PLANT_RELEASE_TIME, true)
    WorldSettings_ChildSpawner_RegenPeriod(inst, TUNING.SDF_PUMPKIN_GOURD_PLANT_REGEN_TIME, true)
...
return Prefab("sdf_pumpkin_gourd_plant", fn, assets, prefabs)
