local assets=
{
    ...
    Asset("ANIM", "anim/sdf_flaming_crossbow.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_crossbow_sdf_standard_bolts.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_crossbow_sdf_flaming_bolts.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_crossbow_empty.zip"),
...
local function OnPickupFn(inst, pickupguy)
	...
	inst.components.inventoryitem.imagename = ("sdf_flaming_crossbow_"..quiverammo.prefab.."")
	inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_crossbow_"..quiverammo.prefab..".xml")
    end
end
...
local function OnProjectileLaunched(inst, attacker, target)
	...
	if item ~= nil then
		...
		item:PushEvent("ammounloaded", {sdf_flaming_crossbow = inst})
	    end
	    item:Remove()
	end
    end
...
local function crossbowPowerAttack(inst, target)
	...
	if inst.components.rechargeable:IsCharged() then
		...
		if stackAmount > TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_SHOOT_AMOUNT then
		    stackAmount = TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_SHOOT_AMOUNT
		end

		--Rapid Fire
		for i = 1, stackAmount, 1 do 
		    inst:DoTaskInTime(((i + TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_SHOOT_SPEED) / 10), function()
		...
		--Cooldown
		inst.components.rechargeable:Discharge(TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_COOLDOWN)

		--Resource Cost
		resourceCost(inst, owner, TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_USAGE)
	    end
	end
    end
end
...
local function AmmoLoaded(inst, ammo)
	...
	inst:DoTaskInTime(1, function()
	    inst.components.inventoryitem.imagename = ("sdf_flaming_crossbow_"..ammo.."")
	    inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_crossbow_"..ammo..".xml")
	    owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_crossbow_"..ammo.."", "swap_sdf_flaming_crossbow")
	end)
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
	inst.components.weapon:SetRange(TUNING.SDF_FLAMING_CROSSBOW_RANGE, TUNING.SDF_FLAMING_CROSSBOW_RANGE + 4)
	owner.components.combat:SetAttackPeriod(TUNING.SDF_FLAMING_CROSSBOW_ATTACK_SPEED)
...
local function AmmoUnloaded(inst)
    local owner = inst.components.inventoryitem:GetGrandOwner()
    if owner ~= nil then
	inst.components.inventoryitem.imagename = "sdf_flaming_crossbow_empty"
	inst.components.inventoryitem.atlasname = "images/inventoryimages/sdf_flaming_crossbow_empty.xml"
	owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_crossbow_empty", "swap_sdf_flaming_crossbow_empty")
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
...
local function OnAmmoLoaded(inst, data)
	...
	if data ~= nil and data.item ~= nil then
	    ...
	    data.item:PushEvent("ammoloaded", {sdf_flaming_crossbow = inst})
	end
    end
end
...
local function OnAmmoUnloaded(inst, data)
	...
	if data ~= nil and data.prev_item ~= nil then
	    data.prev_item:PushEvent("ammounloaded", {sdf_flaming_crossbow = inst})
	end
    end
end
...
local function onequip(inst, owner)
	...
	inst.components.inventoryitem.imagename = ("sdf_flaming_crossbow_"..quiverammo.prefab.."")
	inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_crossbow_"..quiverammo.prefab..".xml")
	owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_crossbow_"..quiverammo.prefab.."", "swap_sdf_flaming_crossbow")
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
	inst.components.weapon:SetRange(TUNING.SDF_FLAMING_CROSSBOW_RANGE, TUNING.SDF_FLAMING_CROSSBOW_RANGE + 4)
	owner.components.combat:SetAttackPeriod(TUNING.SDF_FLAMING_CROSSBOW_ATTACK_SPEED)
...
local function fn()
    ...
    inst.AnimState:SetBank("sdf_flaming_crossbow")
    inst.AnimState:SetBuild("sdf_flaming_crossbow")
    inst.AnimState:PlayAnimation("idle")
    ...
    inst.spelltype = "SDF_FLAMING_CROSSBOW_POWER_ATTACK"
    ...
    inst.components.finiteuses:SetMaxUses(TUNING.SDF_FLAMING_CROSSBOW_DURABILITY)
    inst.components.finiteuses:SetUses(TUNING.SDF_FLAMING_CROSSBOW_DURABILITY)
    inst.components.finiteuses.ignorecombatdurabilityloss = true
    inst.components.finiteuses:SetOnFinished(onfinished)

    inst:AddComponent("container")
    inst.components.container:WidgetSetup("sdf_flaming_crossbow")
    ...
    inst.components.rechargeable:SetChargeTime(TUNING.SDF_FLAMING_CROSSBOW_POWER_ATTACK_COOLDOWN)
...
return  Prefab("common/inventory/sdf_flaming_crossbow", fn, assets)