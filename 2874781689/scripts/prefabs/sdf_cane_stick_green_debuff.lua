...
local function OnAttached(inst, target, followsymbol, followoffset)
    ...
    if target_health and not target_health:IsDead() then
        ...
        if target_combat then
            target_combat:GetAttacked(inst, TUNING.SDF_CANE_STICK_GREEN_ROOT_DAMAGE)
        end
    end
end
...
local STOP_TASK_NAME = "stop_sdf_cane_stick_root_debuff"
...
local function fn()
    ...
    inst.components.timer:StartTimer(STOP_TASK_NAME, TUNING.SDF_CANE_STICK_GREEN_ROOT_TIME)
    --local timer = inst:AddComponent("timer")
    --timer:StartTimer(STOP_TASK_NAME, TUNING.SDF_CANE_STICK_GREEN_ROOT_TIME)
...
return Prefab("sdf_cane_stick_green_debuff", fn, assets)