local assets=
{
    ...
    Asset("ANIM", "anim/sdf_flaming_longbow.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_sdf_standard_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_sdf_flaming_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_sdf_magical_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_drawn_sdf_standard_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_drawn_sdf_flaming_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_drawn_sdf_magical_arrows.zip"),
    Asset("ANIM", "anim/swap_sdf_flaming_longbow_empty.zip"),
...
local function OnPickupFn(inst, pickupguy)
	...
	inst.components.inventoryitem.imagename = ("sdf_flaming_longbow_"..quiverammo.prefab.."")
	inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_longbow_"..quiverammo.prefab..".xml")
    end
end
...
local function OnProjectileLaunched(inst, attacker, target)
	...
	if item ~= nil then
		...
		item:PushEvent("ammounloaded", {sdf_flaming_longbow = inst})
	    end
	    item:Remove()
	end
    end
    ...
    --resource cost
    resourceCost(inst, attacker, TUNING.SDF_FLAMING_LONGBOW_USAGE)
end
...
local function createPowerAttackProjectile(inst, ammo, target, owner)
	...
	projectile.components.weapon:SetDamage(projectile.components.weapon.damage * TUNING.SDF_FLAMING_LONGBOW_POWER_ATTACK_DAMAGE_MULTI)
    end
    --Extra Planar Damage
    if projectile.components.planardamage then
	projectile.components.planardamage:SetBaseDamage(projectile.components.planardamage:GetBaseDamage() * TUNING.SDF_FLAMING_LONGBOW_POWER_ATTACK_DAMAGE_MULTI)
    end
    --AOE Fire
...
local function longbowPowerAttack(inst, target)
	...
	if inst.components.rechargeable:IsCharged() then
		...
		--Cooldown
		inst.components.rechargeable:Discharge(TUNING.SDF_FLAMING_LONGBOW_POWER_ATTACK_COOLDOWN)

		--Resource Cost
		resourceCost(inst, owner, TUNING.SDF_FLAMING_LONGBOW_POWER_ATTACK_USAGE)
	    end
	end
    end
end
...
local function AmmoLoaded(inst, ammo)
    local owner = inst.components.inventoryitem:GetGrandOwner()
    if owner ~= nil then
	inst.components.inventoryitem.imagename = ("sdf_flaming_longbow_"..ammo.."")
	inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_longbow_"..ammo..".xml")
	owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_longbow_"..ammo.."", "swap_sdf_flaming_longbow")
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
	inst.components.weapon:SetRange(TUNING.SDF_FLAMING_LONGBOW_RANGE, TUNING.SDF_FLAMING_LONGBOW_RANGE + 4)
	owner.components.combat:SetAttackPeriod(TUNING.SDF_FLAMING_LONGBOW_ATTACK_SPEED)
...
local function AmmoUnloaded(inst)
    local owner = inst.components.inventoryitem:GetGrandOwner()
    if owner ~= nil then
	inst.components.inventoryitem.imagename = "sdf_flaming_longbow_empty"
	inst.components.inventoryitem.atlasname = "images/inventoryimages/sdf_flaming_longbow_empty.xml"
	owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_longbow_empty", "swap_sdf_flaming_longbow_empty")
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
...
local function OnAmmoLoaded(inst, data)
	...
	if data ~= nil and data.item ~= nil then
	    ...
	    data.item:PushEvent("ammoloaded", {sdf_flaming_longbow = inst})
	end
    end
end
...
local function OnAmmoUnloaded(inst, data)
	...
	if data ~= nil and data.prev_item ~= nil then
	    data.prev_item:PushEvent("ammounloaded", {sdf_flaming_longbow = inst})
	end
    end
end
...
local function onequip(inst, owner)
	...
	inst.components.inventoryitem.imagename = ("sdf_flaming_longbow_"..quiverammo.prefab.."")
	inst.components.inventoryitem.atlasname = ("images/inventoryimages/sdf_flaming_longbow_"..quiverammo.prefab..".xml")
	owner.AnimState:OverrideSymbol("swap_object", "swap_sdf_flaming_longbow_"..quiverammo.prefab.."", "swap_sdf_flaming_longbow")
	owner.AnimState:Show("ARM_carry")
	owner.AnimState:Hide("ARM_normal")
	inst.components.weapon:SetRange(TUNING.SDF_FLAMING_LONGBOW_RANGE, TUNING.SDF_FLAMING_LONGBOW_RANGE + 4)
	owner.components.combat:SetAttackPeriod(TUNING.SDF_FLAMING_LONGBOW_ATTACK_SPEED)
...
local function fn()
    ...
    inst.AnimState:SetBank("sdf_flaming_longbow")
    inst.AnimState:SetBuild("sdf_flaming_longbow")
    inst.AnimState:PlayAnimation("idle")
    ...
    inst.spelltype = "SDF_FLAMING_LONGBOW_POWER_ATTACK"
    ...
    inst.components.finiteuses:SetMaxUses(TUNING.SDF_FLAMING_LONGBOW_DURABILITY)
    inst.components.finiteuses:SetUses(TUNING.SDF_FLAMING_LONGBOW_DURABILITY)
    inst.components.finiteuses.ignorecombatdurabilityloss = true
    inst.components.finiteuses:SetOnFinished(onfinished)

    inst:AddComponent("container")
    inst.components.container:WidgetSetup("sdf_flaming_longbow")
    ...
    inst.components.rechargeable:SetChargeTime(TUNING.SDF_FLAMING_LONGBOW_POWER_ATTACK_COOLDOWN)
...
return  Prefab("common/inventory/sdf_flaming_longbow", fn, assets)