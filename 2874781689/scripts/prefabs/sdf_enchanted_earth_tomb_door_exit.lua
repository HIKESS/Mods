...
local function onAccept(hDc_M, qW0lRiD1, iD1IUx)
    ...
    hDc_M.components.sdf_enchanted_earth_tomb_teleporter:Activate(iD1IUx)
end

local function Vu0cCAf(JLCOx_ak, hPQ)
    if hPQ and hPQ:HasTag("player") then
        JLCOx_ak.components.sdf_enchanted_earth_tomb_teleporter:Activate(hPQ)
    end
end
...
local function q(inst)
    if inst.components.teleporter and inst.components.teleporter.targetTeleporter ~= nil then
        ...
        NsoTwDs.components.sdf_enchanted_earth_tomb_teleporter:Target(inst)
        inst.components.sdf_enchanted_earth_tomb_teleporter:Target(NsoTwDs)
...
local function fn()
    ...
    inst:AddTag("sdf_enchanted_earth_tomb_door_exit")
    ...
    inst.components.sdf_enchanted_earth_tomb_teleporter.onActivate = yxjl
...
local function lqT(inst)
    ...
    TheWorld.Map:CreateSDFEnchantedEarthTomb(x, z, inst)
end

local function onremove(inst)
    TheWorld.Map:RemoveSDFEnchantedEarthTomb(inst)
end
...
local function fnBase()
    ...
    inst:AddTag("NOBLOCK")
    inst:AddTag("antlion_sinkhole_blocker")
    inst:AddTag("sdf_enchanted_earth_tomb_base")
...
return Prefab("sdf_enchanted_earth_tomb_door_exit", fn, assets),
	Prefab("sdf_enchanted_earth_tomb_base", fnBase)
