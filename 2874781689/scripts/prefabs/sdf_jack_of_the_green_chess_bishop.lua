local assets =
{
    Asset("ANIM", "anim/sdf_jack_of_the_green_chess_bishop.zip"),
...
local brain = require "brains/sdf_jack_of_the_green_chess_bishopbrain"
...
SetSharedLootTable("bishop_jack_of_the_green",
{
    {"cutgrass", 1.0},
    {"cutgrass", 1.0},
    {"cutgrass", 1.0},
...
local function Retarget(inst, range)
    ...
    return not (homePos ~= nil and
                inst:GetDistanceSqToPoint(homePos:Get()) >= TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_TARGET_DIST * TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_TARGET_DIST and
                (inst.components.follower == nil or inst.components.follower.leader == nil))
        and FindEntity(
            inst,
            TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_TARGET_DIST,
            function(guy)
                ...
                if not guy:IsNear(inst, TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_TARGET_DIST * CHESSFRIEND_RANGE_PERCENT) and guy:HasTag("chessfriend") then
                    return false
                end
...
local function EquipWeapon(inst)
    if inst.components.inventory ~= nil and not inst.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS) then
        ...
        weapon.components.weapon:SetProjectile("sdf_jack_of_the_green_chess_bishop_charge")
...
local function fn()
    ...
    inst.AnimState:SetBuild("sdf_jack_of_the_green_chess_bishop")
    ...
    inst.components.lootdropper:SetChanceLootTable("bishop_jack_of_the_green")

    inst:AddComponent("locomotor")
    inst.components.locomotor.walkspeed = TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_WALK_SPEED

    inst:SetStateGraph("SGsdf_jack_of_the_green_chess_bishop")
    inst:SetBrain(brain)
    ...
    inst.components.health.fire_damage_scale = TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_FIRE_DAMAGE_SCALE
    inst.components.health:SetMaxHealth(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_HEALTH)
    --inst.components.health:StartRegen(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_HEALTH_REGEN_AMOUNT, TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_HEALTH_REGEN_PERIOD)

    inst:AddComponent("combat")
    inst.components.combat.hiteffectsymbol = "waist"
    inst.components.combat:SetAttackPeriod(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_ATTACK_PERIOD)
    inst.components.combat:SetRange(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_ATTACK_DIST)
    inst.components.combat:SetRetargetFunction(3, Retarget)
    inst.components.combat:SetKeepTargetFunction(KeepTarget)
    inst.components.combat:SetDefaultDamage(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_DAMAGE)
    inst.components.combat:SetAttackPeriod(TUNING.SDF_JACK_OF_THE_GREEN_CHESS_BISHOP_ATTACK_PERIOD)
...
return Prefab("sdf_jack_of_the_green_chess_bishop", fn, assets)
