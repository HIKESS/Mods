...
local function OnFindFire(inst, firePos)
    ...
    if inst:IsAsleep() or inst.components.freezable:IsFrozen() then
...
local function CheckFocusTarget(inst)
	...
	or inst._focustarget:HasTag("player") or inst._focustarget:HasTag("playerghost")) then
...
local RETARGET_CANT_TAGS = { "DECOR", "sdf_pumpkin_king", "sdf_pumpkin_king_vine_end", "sdf_pumpking_friend", "FX", "INLIMBO", "NOCLICK", "notarget", "player", "playerghost", "companion", "summonedbyplayer", "wall" }
...
local function ShareTarget(dude)
    return dude:HasTag("sdf_pumpkin_creeper")
end

local function OnAttacked(inst, data)
    local ftarget = CheckFocusTarget(inst)
    if ftarget == nil and (data.attacker ~= nil and not (data.attacker:HasTag("sdf_pumpking_friend") or data.attacker:HasTag("player"))) then
...
local function WakeUp(inst)
	...
	inst.components.freezable:Thaw(TUNING.SDF_PUMPKIN_CREEPER_FREEZE_TIME * 0.5)
    end
end
...
local function frozenState(inst)
	...
	inst.components.freezable.damagetobreak = TUNING.SDF_PUMPKIN_CREEPER_HEALTH * 0.25
	...
	inst.hibernatetask = inst:DoTaskInTime(TUNING.SDF_PUMPKIN_CREEPER_FREEZE_TIME, frozenState)
    else
...
local function fn()
    ...
    inst:AddTag("sdf_pumpkin_creeper")
    ...
    inst.components.locomotor.runspeed = TUNING.SDF_PUMPKIN_CREEPER_RUNSPEED
    inst.components.locomotor.walkspeed = TUNING.SDF_PUMPKIN_CREEPER_WALKSPEED
    ...
    inst.components.health:SetMaxHealth(TUNING.SDF_PUMPKIN_CREEPER_HEALTH)
    inst.components.health.fire_damage_scale = TUNING.SDF_PUMPKIN_CREEPER_FIRE_DAMAGE

    inst:AddComponent("combat")
    inst.components.combat:SetDefaultDamage(TUNING.SDF_PUMPKIN_CREEPER_DAMAGE)
    inst.components.combat:SetAttackPeriod(TUNING.SDF_PUMPKIN_CREEPER_ATTACK_PERIOD)
    inst.components.combat:SetRetargetFunction(2, Retarget)
    inst.components.combat:SetKeepTargetFunction(KeepTarget)
    inst.components.combat:SetHurtSound(inst.sounds.hurt)
    inst.components.combat:SetRange(TUNING.SDF_PUMPKIN_CREEPER_TARGET_RANGE, TUNING.SDF_PUMPKIN_CREEPER_ATTACK_RANGE)
    inst.components.combat:EnableAreaDamage(true)
    inst.components.combat:SetAreaDamage(TUNING.SDF_PUMPKIN_CREEPER_ATTACK_RANGE, 1, function(ent, inst)
...
return Prefab("sdf_pumpkin_creeper", fn, assets, prefabs)