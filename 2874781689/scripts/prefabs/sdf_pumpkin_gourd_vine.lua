...
local RETARGET_CANT_TAGS = { "player", "playerghost", "companion", "summonedbyplayer", "sdf_pumpking_friend", "prey", "INLIMBO" }
local RETARGET_ONEOF_TAGS = { "character", "monster", "animal" }
local function retargetfn(inst)
    return FindEntity(inst,
            TUNING.SDF_PUMPKIN_GOURD_VINE_ATTACK_DIST,
            IsAlive,
            RETARGET_MUST_TAGS,
            RETARGET_CANT_TAGS,
            RETARGET_ONEOF_TAGS
...
local function ShouldKeepTarget(inst, target)
    if target and target:IsValid() and target.components.health and not target.components.health:IsDead() then
        ...
        return distsq < TUNING.SDF_PUMPKIN_GOURD_VINE_STOPATTACK_DIST * TUNING.SDF_PUMPKIN_GOURD_VINE_STOPATTACK_DIST
    else
	Retract(inst)
        return false
    end
...
local function fn()
    ...
    inst:AddTag("sdf_pumpkin_gourd_vine")
    ...
    inst.components.health:SetMaxHealth(TUNING.SDF_PUMPKIN_GOURD_VINE_HEALTH)
    inst.components.health.fire_damage_scale = TUNING.SDF_PUMPKIN_GOURD_FIRE_DAMAGE

    inst:AddComponent("combat")
    inst.components.combat:SetRange(TUNING.SDF_PUMPKIN_GOURD_VINE_ATTACK_DIST)
    inst.components.combat:SetDefaultDamage(TUNING.SDF_PUMPKIN_GOURD_VINE_DAMAGE)
    inst.components.combat:SetAttackPeriod(TUNING.SDF_PUMPKIN_GOURD_VINE_ATTACK_PERIOD)
    ...
    inst.lifeSpan = TUNING.SDF_PUMPKIN_GOURD_VINE_LIFE_SPAN
    inst.retracted = true
    inst.Emerge = Emerge
...
return Prefab("sdf_pumpkin_gourd_vine", fn, assets, prefabs)