...
local items =
{
    SWORD = ("swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_WEAPON..""),
    SHIELD = ("swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_SHIELD..""),
}
...
local function GetWeaponDamage(weapon)
    if weapon == "sdf_broad_sword" then
	return TUNING.SDF_BROAD_SWORD_DAMAGE
    elseif weapon == "sdf_small_sword" then
	return TUNING.SDF_SMALL_SWORD_DAMAGE
    else
	return TUNING.SDF_GALLOWMERE_KNIGHT_ATTACK
    end
end

local function GetShieldProtection(shield)
    if shield == "sdf_silver_shield" then
	return TUNING.SDF_GALLOWMERE_KNIGHT_SILVER_DEFENSE
    elseif shield == "sdf_copper_shield" then
	return TUNING.SDF_GALLOWMERE_KNIGHT_COPPER_DEFENSE
    else
	return TUNING.SDF_GALLOWMERE_KNIGHT_DEFENSE
    end
end
...
local function NormalRetarget(inst)
    ...
    if inst:HasTag("sdf_gallowmere_knight_command_stay") then
	return FindEntity(inst, TARGET_DIST, 
	    function(guy) 
		if inst.components.combat:CanTarget(guy) then
		    return guy:HasTag("shadowcreature") or guy:HasTag("monster") or guy:HasTag("hostile") or guy:HasTag("brightmare") or guy:HasTag("lunar_aligned") or guy:HasTag("shadow_aligned") or (guy.components.combat.target and guy.components.combat.target:HasTag("player"))
		end
...
local function gknighton(inst, player)
	...
	--Order gallowmere knight
	inst:AddTag("sdf_gallowmere_knight_tactics")
    end
...
local function OnCallToArms(inst, data)
    if inst.components.sdf_gallowmere_knight_command then
	if inst.components.sdf_gallowmere_knight_unteleportable == nil then
	    inst:AddComponent("sdf_gallowmere_knight_unteleportable")
	end

	inst:DoTaskInTime(0.1, function(inst)
	    inst.components.sdf_gallowmere_knight_command:CallToArms()
	    inst:AddTag("sdf_gallowmere_knight_command_stay")
	    inst:RemoveTag("sdf_gallowmere_knight_command_follow")
	end)
    end
end
...
local function onload(inst, data)
    ...
    if stance == ("swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_SHIELD.."") and staying == true then
	inst.AnimState:OverrideSymbol("lantern_overlay", "swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_SHIELD.."", "swap_shield")
	inst.AnimState:HideSymbol("swap_object")
	inst.AnimState:Show("LANTERN_OVERLAY")
    else
	inst.AnimState:ClearOverrideSymbol("lantern_overlay")
	inst.AnimState:OverrideSymbol("swap_object", "swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_WEAPON.."", "swap_"..TUNING.SDF_GALLOWMERE_KNIGHT_WEAPON.."")
	inst.AnimState:Show("ARM_carry") 
	inst.AnimState:Hide("ARM_normal")
	inst.AnimState:Hide("LANTERN_OVERLAY")
	inst.AnimState:ShowSymbol("swap_object")
...
local function fn()
    ...
    inst.components.health:SetMaxHealth(TUNING.SDF_GALLOWMERE_KNIGHT_HEALTH)
    inst.components.health.canheal = false
    inst.components.health.canmurder = false
    inst.components.health:SetAbsorptionAmount(GetShieldProtection(TUNING.SDF_GALLOWMERE_KNIGHT_SHIELD)) --sdf_silver_shield 0.75
    ...
    inst.components.combat:SetDefaultDamage(GetWeaponDamage(TUNING.SDF_GALLOWMERE_KNIGHT_WEAPON)) --sdf_small_sword 27.5
    inst.components.combat:SetAttackPeriod(TUNING.SDF_GALLOWMERE_KNIGHT_ATTACK_SPEED + math.random() * 2) --3
    ...
    inst.components.locomotor.runspeed = TUNING.SDF_GALLOWMERE_KNIGHT_MOVEMENT_SPEED --7
    ...
    inst.components.lootdropper:AddChanceLoot("sdf_energyvial", TUNING.SDF_GALLOWMERE_KNIGHT_ENERGYVIAL_CHANCE)
    ...
    inst.components.burnable.flammability = TUNING.SDF_GALLOWMERE_KNIGHT_FLAMMABILITY
    ...
    inst.items = items
    inst.weaponfn = GetWeaponDamage(TUNING.SDF_GALLOWMERE_KNIGHT_WEAPON)
    inst.shieldfn = GetShieldProtection(TUNING.SDF_GALLOWMERE_KNIGHT_SHIELD)
    ...
    inst:ListenForEvent("sdf_king_peregrins_crown_calltoarms", OnCallToArms)
...
return Prefab("sdf_gallowmere_knight", fn, assets, prefabs)