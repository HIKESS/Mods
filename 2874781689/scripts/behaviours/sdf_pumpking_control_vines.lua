SDFPumpking_Gourd_Control_Vines = Class(BehaviourNode, function(self, inst)
    BehaviourNode._ctor(self, "ControlMinions")
    self.inst = inst
    self.ms = inst.components.sdf_pumpking_gourd_vine_spawner
    self.radius = nil
    self.minionrange = 3.5
end)

function SDFPumpking_Gourd_Control_Vines:GetClosestMinion(item, minions)
...
function SDFPumpking_Gourd_Control_Vines:Visit()
