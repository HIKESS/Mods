...
--Replace Anubis Stone Part 1
local ACTIONS = GLOBAL.ACTIONS
local ActionHandler = GLOBAL.ActionHandler

local id = "SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1"
local name = STRINGS.ACTIONHANDLER_SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1


local fn = function(act)

    if act.doer.prefab == "sdf" and act.target.INFORMATION_ON == true then
	if act.doer.components.sdf_anubis_stone_quest:GetAnubisStonePart1FoundStatus() == true then
		...
		act.target:DoTaskInTime(0.5, function()
			...
			act.target:DoTaskInTime(0.1, function()
			    ...
			    local aSPart= SpawnPrefab("sdf_anubis_stone_part1")

			    --Destory all old Anubis Stone Part
			    local oldAnubisStonePart = act.doer.components.sdf_key_item_inventory:GetKeyItem("sdf_anubis_stone_part1")
			...
			if act.doer.components.talker then
			    act.doer.components.talker:Say(GetString(act.doer, "ANNOUNCE_SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1"))
			end
		    end
		end)
	    end)
...
local type = "USEITEM"
local component = "sdf_witch_talisman_offering_information_gargoyle_mcm"
local testfn = function(inst, doer, target, actions)
    if target:HasTag("sdf_witch_talisman_offering") and target:HasTag("sdf_information_gargoyle_4") then
	table.insert(actions, ACTIONS.SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1)
    end
end
...
AddStategraphActionHandler("wilson",ActionHandler(ACTIONS.SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1, state))
AddStategraphActionHandler("wilson_client",ActionHandler(ACTIONS.SDF_WITCH_TALISMAN_OFFERING_ANUBIS_STONE_PART1,state))