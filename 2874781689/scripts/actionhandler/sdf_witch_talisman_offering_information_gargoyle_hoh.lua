...
--Replace King Peregrins Crown
local ACTIONS = GLOBAL.ACTIONS
local ActionHandler = GLOBAL.ActionHandler

local id = "SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN"
local name = STRINGS.ACTIONHANDLER_SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN
	...
	if act.doer.components.sdf_king_peregrin_quest:GetCrownOfferedStatus() == true then
		...
		act.target:DoTaskInTime(0.5, function()
			...
			act.target:DoTaskInTime(0.1, function()
			    ...
			    local kingCrown= SpawnPrefab("sdf_king_peregrins_crown")
			    ...
			    act.doer.components.sdf_key_item_inventory:SetKeyItem(kingCrown, act.doer)
			    if holder ~= nil then
				local slot = holder:GetItemSlot(act.invobject)
				act.invobject:Remove()
				holder:GiveItem(kingCrown, slot)
			    end
			end)

			if act.doer.components.talker then
			    act.doer.components.talker:Say(GetString(act.doer, "ANNOUNCE_SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN"))
			end
		    end
		end)
	    end)
...
local type = "USEITEM"
local component = "sdf_witch_talisman_offering_information_gargoyle_hoh"
local testfn = function(inst, doer, target, actions)
    if target:HasTag("sdf_witch_talisman_offering") and target:HasTag("sdf_information_gargoyle_1") then
	table.insert(actions, ACTIONS.SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN)
    end
end
...
AddStategraphActionHandler("wilson",ActionHandler(ACTIONS.SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN, state))
AddStategraphActionHandler("wilson_client",ActionHandler(ACTIONS.SDF_WITCH_TALISMAN_OFFERING_KING_PEREGRINS_CROWN,state))