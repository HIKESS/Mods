local SDFDaring_Dash = Class(function(self, inst)
    self.inst = inst
...
function SDFDaring_Dash:SetOnStartTackleFn(fn)
    self.onstarttacklefn = fn
end

function SDFDaring_Dash:StartTackle()
    return self.onstarttacklefn ~= nil and self.onstarttacklefn(self.inst)
end

function SDFDaring_Dash:SetDistance(distance)
    self.distance = distance
end

function SDFDaring_Dash:SetRadius(radius)
    self.radius = radius
end

function SDFDaring_Dash:SetStructureDamageMultiplier(mult)
    self.structure_damage_mult = mult
end

function SDFDaring_Dash:AddWorkAction(action, amount)
...
function SDFDaring_Dash:SetOnCollideFn(fn)
    self.oncollidefn = fn
end

function SDFDaring_Dash:SetOnTrampleFn(fn)
    self.ontramplefn = fn
end

function SDFDaring_Dash:CheckCollision(ignores)
    ...
    for i, v in ipairs(TheSim:FindEntities(x1, 0, z1, self.radius + 3, nil, self.no_collide_tags, self.collide_tags)) do
        if ignores == nil or not ignores[v] then
			...
			and not (self.inst.TargetForceAttackOnly ~= nil and self.inst:TargetForceAttackOnly(v)) then
...
function SDFDaring_Dash:SetEdgeDistance(distance)
...
function SDFDaring_Dash:CheckEdge()
    local x, y, z = self.inst.Transform:GetWorldPosition()
