...
local SDFPumpkin_Gorge_Resource_Spawner = Class(function(self, inst)
    self.inst = inst
    self.miniontype = "sdf_pumpkin_gorge_farmland_debris"
    self.maxminions = 6
    self.maxminionspool = 8
    self.minionspawntime = { min = 1, max = 2 }
    self.minions = {}
    self.numminions = 0
    self.distancemodifier = 15
...
function SDFPumpkin_Gorge_Resource_Spawner:SetMinionType(minionType)
    self.miniontype = minionType
end 

function SDFPumpkin_Gorge_Resource_Spawner:SetMaxMinion(num)
    self.maxminions = num
end 

function SDFPumpkin_Gorge_Resource_Spawner:SetMaxMinionPool(num)
    self.maxminionspool = num
end 

function SDFPumpkin_Gorge_Resource_Spawner:SetDistanceModifier(num)
    self.distancemodifier = num
end 

function SDFPumpkin_Gorge_Resource_Spawner:SetSpawnTime(num)
    self.spawntime = num
end 

function SDFPumpkin_Gorge_Resource_Spawner:SetMinionSpawnTime(num)
    self.minionspawntime = num
end 

function SDFPumpkin_Gorge_Resource_Spawner:GetDebugString()
...
function SDFPumpkin_Gorge_Resource_Spawner:RemovePosition(num)
...
function SDFPumpkin_Gorge_Resource_Spawner:AddPosition(num, tbl)
...
function SDFPumpkin_Gorge_Resource_Spawner:OnSave()
    local data = {}
    local guidtable = {}
...
function SDFPumpkin_Gorge_Resource_Spawner:OnLoad(data)
...
function SDFPumpkin_Gorge_Resource_Spawner:LoadPostPass(newents, savedata)
...
function SDFPumpkin_Gorge_Resource_Spawner:TakeOwnership(minion)
...
function SDFPumpkin_Gorge_Resource_Spawner:OnLostMinion(minion)
    if self.minions[minion] == nil then
        return
    end
...
function SDFPumpkin_Gorge_Resource_Spawner:MakeMinion()
    if self.miniontype ~= nil and not self:MaxedMinions() then
        return SpawnPrefab(self.miniontype)
    end
end

function SDFPumpkin_Gorge_Resource_Spawner:CheckTileCompatibility(tile)
    return self.validtiletypes[tile]
end

local FARMLAND_TAGS = { "sdf_pumpkin_gorge_resource" }
function SDFPumpkin_Gorge_Resource_Spawner:MakeSpawnLocations()
    ...
    for i = 1, 100 do
        ...
        if ground.Map:IsAboveGroundAtPoint(pos:Get()) and
            ...
            #TheSim:FindEntities(pos.x, pos.y, pos.z, 2.5, FARMLAND_TAGS) <= 0 and
...
function SDFPumpkin_Gorge_Resource_Spawner:GetSpawnLocation(num)
    if self.minionpositions == nil then
        return
    end
...
function SDFPumpkin_Gorge_Resource_Spawner:GetNextSpawnTime()
...
function SDFPumpkin_Gorge_Resource_Spawner:KillAllMinions()
    self.spawninprogress = false
    for k, v in pairs(self.minions) do
...
function SDFPumpkin_Gorge_Resource_Spawner:SpawnNewMinion()
    if self.minionpositions == nil then
...
function SDFPumpkin_Gorge_Resource_Spawner:MaxedMinions()
    return self.numminions >= self.maxminions
end

function SDFPumpkin_Gorge_Resource_Spawner:SetSpawnInfo(time)
...
function SDFPumpkin_Gorge_Resource_Spawner:StartNextSpawn()
...
function SDFPumpkin_Gorge_Resource_Spawner:ResumeSpawn(time)
    self.spawninprogress = true
...
function SDFPumpkin_Gorge_Resource_Spawner:LongUpdate(dt)
...
return SDFPumpkin_Gorge_Resource_Spawner
