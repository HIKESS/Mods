local function fuZ3z86(yxjl)
	...
	yxjl.inst:AddTag("sdf_enchanted_earth_tomb_teleporter")
    else 
	yxjl.inst:RemoveTag("sdf_enchanted_earth_tomb_teleporter")
    end
end;

local SDFEnchanted_Earth_Tomb_Teleporter=Class(function(self,inst)
    self.inst=inst;
...
nil,
{
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:OnRemoveFromEntity()
	self.inst:RemoveTag("sdf_enchanted_earth_tomb_teleporter")
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:IsActive()
	return self.enabled and self.targetTeleporter~=nil
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:IsBusy()
	return self.numteleporting>0 or next(self.teleportees)~=nil
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:IsTargetBusy()
	return self.targetTeleporter~=nil and self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter:IsBusy()
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:RegisterTeleportee(kP7O5)
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:UnregisterTeleportee(lqT)
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:Activate(mP3mlD)
	...
	if self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter~=nil then
	    if self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter.onActivateByOther~=nil then
		self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter.onActivateByOther(self.targetTeleporter,self.inst,mP3mlD)
	    end;
	    self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter.numteleporting=self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter.numteleporting+1
	end;
	self:Teleport(mP3mlD)
	if self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter~=nil then
	    if mP3mlD:HasTag("player")then
		self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter:ReceivePlayer(mP3mlD)
	    elseif mP3mlD.components.inventoryitem~=nil then
		self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter:ReceiveItem(mP3mlD)
	    end
	end;
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:Teleport(iy)
	if self.targetTeleporter~=nil then
	    local m6SCS0,NUhYw6R4,Hv=self.targetTeleporter.Transform:GetWorldPosition()
	    local Ch= self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter~=nil and self.targetTeleporter.components.sdf_enchanted_earth_tomb_teleporter.offset or 0;
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:PushDoneTeleporting(JZAU2)
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:ReceiveItem(qboV)
	...
	if not zz1QI:IsValid()then
	    zz1QI=nil
	elseif zz1QI.sg.statemem.sdf_enchanted_earth_tomb_teleportarrivestate~=nil then
	    zz1QI.sg:GoToState(zz1QI.sg.statemem.sdf_enchanted_earth_tomb_teleportarrivestate)
	end;
	i1.numteleporting=i1.numteleporting-1;
	i1:PushDoneTeleporting(zz1QI)
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:ReceivePlayer(kFTAh)
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:Target(LBf)
	self.targetTeleporter=LBf
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:SetEnabled(dijn4Ph)
	self.enabled=dijn4Ph
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:OnSave()
    ...
    function SDFEnchanted_Earth_Tomb_Teleporter:LoadPostPass(CO1,RlZo)
	if RlZo~=nil and RlZo.target~=nil then
	    local SUn=CO1[RlZo.target]
	    if SUn~=nil and SUn.entity.components.sdf_enchanted_earth_tomb_teleporter~=nil then
		self.targetTeleporter=SUn.entity
	    end
	end
    end;

    function SDFEnchanted_Earth_Tomb_Teleporter:GetDebugString()
	return"Enabled: ".. (self.enabled and"T"or"F").." Target:"..tostring(self.targetTeleporter)
    end;

return SDFEnchanted_Earth_Tomb_Teleporter