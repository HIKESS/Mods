local function fuZ3z86(yxjl)
	...
	yxjl.inst:AddTag("sdf_professors_lab_teleporter")
    else 
	yxjl.inst:RemoveTag("sdf_professors_lab_teleporter")
    end
end;

local SDFProfessors_Lab_Teleporter=Class(function(self,inst)
    self.inst=inst;
...
nil,
{
    ...
    function SDFProfessors_Lab_Teleporter:OnRemoveFromEntity()
	self.inst:RemoveTag("sdf_professors_lab_teleporter")
    end;

    function SDFProfessors_Lab_Teleporter:IsActive()
	return self.enabled and self.targetTeleporter~=nil
    end;

    function SDFProfessors_Lab_Teleporter:IsBusy()
	return self.numteleporting>0 or next(self.teleportees)~=nil
    end;

    function SDFProfessors_Lab_Teleporter:IsTargetBusy()
	return self.targetTeleporter~=nil and self.targetTeleporter.components.sdf_professors_lab_teleporter:IsBusy()
    end;

    function SDFProfessors_Lab_Teleporter:RegisterTeleportee(kP7O5)
    ...
    function SDFProfessors_Lab_Teleporter:UnregisterTeleportee(lqT)
    ...
    function SDFProfessors_Lab_Teleporter:Activate(mP3mlD)
	...
	if self.targetTeleporter.components.sdf_professors_lab_teleporter~=nil then
	    if self.targetTeleporter.components.sdf_professors_lab_teleporter.onActivateByOther~=nil then
		self.targetTeleporter.components.sdf_professors_lab_teleporter.onActivateByOther(self.targetTeleporter,self.inst,mP3mlD)
	    end;
	    self.targetTeleporter.components.sdf_professors_lab_teleporter.numteleporting=self.targetTeleporter.components.sdf_professors_lab_teleporter.numteleporting+1
	end;
	self:Teleport(mP3mlD)
	if self.targetTeleporter.components.sdf_professors_lab_teleporter~=nil then
	    if mP3mlD:HasTag("player")then
		self.targetTeleporter.components.sdf_professors_lab_teleporter:ReceivePlayer(mP3mlD)
	    elseif mP3mlD.components.inventoryitem~=nil then
		self.targetTeleporter.components.sdf_professors_lab_teleporter:ReceiveItem(mP3mlD)
	    end
	end;
    ...
    function SDFProfessors_Lab_Teleporter:Teleport(iy)
	if self.targetTeleporter~=nil then
	    local m6SCS0,NUhYw6R4,Hv=self.targetTeleporter.Transform:GetWorldPosition()
	    local Ch= self.targetTeleporter.components.sdf_professors_lab_teleporter~=nil and self.targetTeleporter.components.sdf_professors_lab_teleporter.offset or 0;
    ...
    function SDFProfessors_Lab_Teleporter:PushDoneTeleporting(JZAU2)
    ...
    function SDFProfessors_Lab_Teleporter:ReceiveItem(qboV)
	...
	if not zz1QI:IsValid()then
	    zz1QI=nil
	elseif zz1QI.sg.statemem.sdf_professors_lab_teleportarrivestate~=nil then
	    zz1QI.sg:GoToState(zz1QI.sg.statemem.sdf_professors_lab_teleportarrivestate)
	end;
	i1.numteleporting=i1.numteleporting-1;
	i1:PushDoneTeleporting(zz1QI)
    end;

    function SDFProfessors_Lab_Teleporter:ReceivePlayer(kFTAh)
    ...
    function SDFProfessors_Lab_Teleporter:Target(LBf)
	self.targetTeleporter=LBf
    end;

    function SDFProfessors_Lab_Teleporter:SetEnabled(dijn4Ph)
	self.enabled=dijn4Ph
    end;

    function SDFProfessors_Lab_Teleporter:OnSave()
    ...
    function SDFProfessors_Lab_Teleporter:LoadPostPass(CO1,RlZo)
	if RlZo~=nil and RlZo.target~=nil then
	    local SUn=CO1[RlZo.target]
	    if SUn~=nil and SUn.entity.components.sdf_professors_lab_teleporter~=nil then
		self.targetTeleporter=SUn.entity
	    end
	end
    end;

    function SDFProfessors_Lab_Teleporter:GetDebugString()
	return"Enabled: ".. (self.enabled and"T"or"F").." Target:"..tostring(self.targetTeleporter)
    end;

return SDFProfessors_Lab_Teleporter