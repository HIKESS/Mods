...
local PHYSICS_RADIUS = 1.1
local DAMAGE_RADIUS_PADDING = 1.2
...
local function StartSpike(inst, duration, variation)
    ...
    inst.basefx = SpawnPrefab("gwen_spikebase")
...
local function fn()
    ...
    inst.AnimState:SetMultColour(0.1, 0.7, 0.4, 0.4)
    ...
    inst.task = inst:DoTaskInTime(0, StartSpike, 8 + math.random(), math.random(NUM_VARIATIONS))
...
local function basefn()
    ...
    inst.AnimState:SetMultColour(0.1, 0.7, 0.4, 0.5)
...
return Prefab("gwen_spike", fn, assets, prefabs),
    Prefab("gwen_spikebase", basefn, assets)