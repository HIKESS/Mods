...
local function onequip_yifu(inst, owner)
	owner.AnimState:OverrideSymbol("swap_body", "gw_yifu_zhandou", "swap_body")
    inst:ListenForEvent("blocked", OnBlocked, owner)
    if not owner:HasTag("gw_cs_1") then
		owner:AddTag("gw_cs_1")
	end
    if owner and owner.components.inventory then
        for k,v in pairs(owner.components.inventory.equipslots) do
            if v and v.prefab == "gw_maozi_zhandou" then
                owner.gw_taozhuang_zhandou_active = true
            end
        end
    end
end
...
local function onunequip_yifu(inst, owner)
    ...
    owner:RemoveTag("gw_cs_1")

    if owner.gw_taozhuang_zhandou_active ~= nil then
        owner.gw_taozhuang_zhandou_active = nil
    end
end
...
local function onequip_maozi(inst, owner) 
	owner.AnimState:OverrideSymbol("swap_hat", "gw_maozi_zhandou", "swap_hat")
    ...
    if owner and owner.components.inventory then
        for k,v in pairs(owner.components.inventory.equipslots) do
            if v and v.prefab == "gw_yifu_zhandou" then
                owner.gw_taozhuang_zhandou_active = true
            end
        end
    end

	if not owner:HasTag("zhandoukin") then
		owner:AddTag("zhandoukin")
	end
...
local function tryproc(inst, owner, data)
    if owner.gw_taozhuang_zhandou_active == true and inst._task == nil and
...
local function gw_yifufn()
    ...
    inst.AnimState:SetBank("gw_yifu_zhandou")
    inst.AnimState:SetBuild("gw_yifu_zhandou")
	...
	inst.components.inventoryitem.atlasname = "images/inventoryimages/gw_yifu_zhandou.xml"
...
local function gw_maozifn(Sim)
    ...
    inst.AnimState:SetBank("gw_maozi_zhandou")
    inst.AnimState:SetBuild("gw_maozi_zhandou")
    inst.AnimState:PlayAnimation("anim")
    ...
    inst:AddComponent("inventoryitem")
	inst.components.inventoryitem.atlasname = "images/inventoryimages/gw_maozi_zhandou.xml"
	...
	-- inst.components.fueled:InitializeFuelLevel(TUNING.BEEFALOHAT_PERISHTIME*0.5)
    ...
    -- inst.components.container:WidgetSetup("gw_maozi_zhandou")
...
----------------------------------------------------------------------
return Prefab("gw_yifu_zhandou", gw_yifufn, assets),
		Prefab("gw_maozi_zhandou", gw_maozifn, assets)