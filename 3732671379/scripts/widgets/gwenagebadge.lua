...
local LastingAgeBadge = Class(Badge, function(self, owner)
    Badge._ctor(self, "status_lasting", owner, { .8, .8, .8, 1 }, nil, nil, nil, true)
    ...
    self.year_hand:GetAnimState():SetBank("status_lasting")
    self.year_hand:GetAnimState():SetBuild("status_lasting")
    ...
    self.days_hand:GetAnimState():SetBank("status_lasting")
    self.days_hand:GetAnimState():SetBuild("status_lasting")
...
function LastingAgeBadge:ShowEffigy()
...
function LastingAgeBadge:HideEffigy()
    if self.effigy then
        self.effigy = false
        self.effigyanim:GetAnimState():PlayAnimation("effigy_deactivate")
...
function LastingAgeBadge:SetPercent(val, max, penaltypercent)
    -- 现在val参数直接代表生命值百分比
    self.health_precent = val

    -- 更新显示文本为生命百分比
    self.num:SetString(tostring(math.floor(val * 100 + 0.5)) .. "%")
end

function LastingAgeBadge:OnUpdate(dt)
...
function LastingAgeBadge:PulseColor(r, g, b, a)
...
function LastingAgeBadge:PulseGreen()
    self:PulseColor(0, 1, 0, 1)
end

function LastingAgeBadge:PulseRed()
    self:PulseColor(1, 0, 0, 1)
end

function LastingAgeBadge:PulseOff()
...
function LastingAgeBadge:Pulse(color)
...
function LastingAgeBadge:ShowBuff(symbol)
end

function LastingAgeBadge:UpdateBuff(symbol)
    self:ShowBuff(symbol)
end

function LastingAgeBadge:ShowEffigy(effigy_type)
...
function LastingAgeBadge:HealthDelta(data)
